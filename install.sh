...
command -v headroom >/dev/null 2>&1 || {
	error "headroom not installed. Run ./setup.sh."
	exit 1
}
command -v hermes >/dev/null 2>&1 || {
	error "hermes not installed. See github.com/NousResearch/hermes-agent"
	exit 1
}
command -v uv >/dev/null 2>&1 || {
	error "uv not installed. Run ./setup.sh."
	exit 1
}
...
if [[ -f ${STATE_FILE} ]]; then
	...
	uv run --quiet --with ruamel.yaml -- python "${SCRIPT_DIR}/lib/configure_hermes.py" \
		--config "${HOME}/.hermes/config.yaml" --state-file "${STATE_FILE}" \
