...
if command -v hermes >/dev/null 2>&1; then
  success "hermes found: $(hermes --version 2>/dev/null || echo '?')"
else
...
if command -v uv >/dev/null 2>&1; then
  success "uv found: $(uv --version 2>/dev/null || echo '?')"
else
...
if command -v headroom >/dev/null 2>&1; then
  success "headroom found: $(headroom --version 2>/dev/null || echo '?')"
else
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  ctx="$(ask 'Context window in tokens [Enter for 1000000]:')"; ctx="${ctx:-1000000}"
...
echo ""
info "Check savings any time:    headroom stats"
